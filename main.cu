    ...
    int numThreads)
{
    ...
    int customSize = totalCubes / totalThreads;

    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int index = idx * customSize;

    int ix = index / (numX * numY);
    int iy = (index/numZ) % numY;
    int iz = index % numX;
    int asdf = 0;

    while (asdf<customSize){
        if (ix < numX && iy < numY && iz < numZ)
        {
            ...
            float3 pos[8];
            float sdf[8];
            float3 intersect[12]; // Define intersect array
        ...
        // printf("while içi: ix=%d, iy=%d, iz=%d, asdf=%d, threadID=%d, BlockID=%d\n\n",ix,iy,iz,totalCubes,threadIdx.x,blockIdx.x);
        if(ix > numX){
            break;
        }
    }
    ...
    int numThreads)
{
    ...
    for(int frame = 1; frame < numFrames+1; frame++){
        ...
        meshNormals += totalCubes * 16;
        meshVertices += totalCubes * 16;
...
int main(int argc, char *argv[])
{
    ...
    ///////////////////////////////////////////////////////////////////
//      float3 *meshVertices_h, *meshNormals_h;
...
//     float3 *meshVertices_d, *meshNormals_d;
// Rect3 *domain_d;
// float3 *cubeSize_d;
...
// Allocate required memory and buffers
float3 *meshVertices_h, *meshNormals_h;
checkCudaErrors(cudaMallocHost(&meshVertices_h, frameSize * frameNum * sizeof(float3)));
checkCudaErrors(cudaMallocHost(&meshNormals_h, frameSize * frameNum * sizeof(float3)));
...
// Allocate memory for single-frame processing (Part 1 & 2)
// If processing one frame at a time, uncomment these lines
// cudaMalloc(&meshVertices_d, frameSize * sizeof(float3));
// cudaMalloc(&meshNormals_d, frameSize * sizeof(float3));

// Allocate memory for multi-frame processing (Part 3)
// If processing multiple frames simultaneously, uncomment these lines
    ...
    if (correctTest || part < 1)
    {
        ...
        for (frame = 0; frame < frameNum; frame++)
        {
            ...
            if (saveObj)
            {
                ...
                WriteObjFile(frameSize, meshVertices_test + offset, meshVertices_test + offset, filename);
            }
...
cudaMalloc(&meshNormals_d, frameSize * sizeof(float3));
            ...
            MarchCubeCUDA<<<numBlocks, numThreads>>>(domain_d, cubeSize_d, twist, 0, meshVertices_d, meshNormals_d, numBlocks, numThreads);
...
cudaMalloc(&meshVertices_d, frameSize * frameNum * sizeof(float3));
cudaMalloc(&meshNormals_d, frameSize * frameNum * sizeof(float3));
MarchCubeCUDAMultiframe<<<numBlocks, numThreads>>>(domain_d, cubeSize_d, maxTwist, 0, frameNum, meshVertices_d, meshNormals_d,numBlocks, numThreads );
